...
public abstract class DiscordBot {
	...
	private EnumStatus status = EnumStatus.ONLINE;
	...
	public final void shutdown() {
		...
		setStatus(EnumStatus.INVISIBLE);
		_client.logout();
		System.exit(0);
	}
	...
	public final void setStatus(EnumStatus status) {
		this.status = status;
		_client.changePresence(status.dcs);
	}
	...
	public final void setListening(String msg) {
		_client.changePresence(status.dcs, ActivityType.LISTENING, msg);
	}
	
	public final void setPlaying(String msg) {
		_client.changePresence(status.dcs, ActivityType.PLAYING, msg);
	}
	
	public final void setWatching(String msg) {
		_client.changePresence(status.dcs, ActivityType.WATCHING, msg);
	}
	...
	private void checkVersion() {
		try {
			...
			Scanner scanner = new Scanner(new URL("").openStream(), "UTF-8").useDelimiter("\\A");
			String out = scanner.next();
			scanner.close();
			if(!out.equals(API_VERSION)) {
				//alert of update
			}
		} 
		catch(Exception e) {
